# BreezeApp Engine - Android 實作

BreezeApp Engine 是一個模組化、可擴充的 AI 推論引擎，專為 Android 平台設計。它提供統一的 API 來整合多種 AI 能力（LLM、ASR、TTS、VLM）並支援不同的硬體後端（CPU、NPU、GPU）。

# 目錄
[English](./README.md) | [繁體中文](./README_zh.md)
- [系統架構總覽](#1-系統架構總覽)
- [服務層架構 (Service Layer)](#2-服務層架構-service-layer)
- [核心業務邏輯層 (Business Logic Layer)](#3-核心業務邏輯層-business-logic-layer)
- [Runner 實作架構 (Runner Implementations)](#4-runner-實作架構-runner-implementations)
- [模型管理系統 (Model Management)](#5-模型管理系統-model-management)
- [安全機制 - Guardian Pipeline](#6-安全機制---guardian-pipeline)
- [參數配置系統](#7-參數配置系統)
- [系統資源管理](#8-系統資源管理)
- [完整推論處理流程](#9-完整推論處理流程)
- [支援的 AI Runners](#10-支援的-ai-runners)
- [部署與整合](#11-部署與整合)
- [開發擴充指南](#12-開發擴充指南)
- [總結](#13-總結)
...
本系統基於 Clean Architecture 原則，實作完整的 AI 推論服務架構：
...
## 2. 服務層架構 (Service Layer)

### 2.1 BreezeAppEngineService - Android 服務核心
...
## 3. 核心業務邏輯層 (Business Logic Layer)

### 3.1 AIEngineManager - 推論處理核心
...
### 3.2 RunnerManager - Runner 生命週期管理
...
## 4. Runner 實作架構 (Runner Implementations)
...
## 5. 模型管理系統 (Model Management)

### 5.1 ModelManager 統一管理
...
系統目前支援多種 AI Runner，每個 Runner 針對不同的 AI 能力進行了最佳化：
...
val request = InferenceRequest(
    ...
    params = mapOf(
        "model_id" to "llama-3.2-3b-instruct",  // 覆寫模型
        "temperature" to 0.9f,                  // 覆寫溫度
        "max_tokens" to 2048,                   // 覆寫生成長度
        "enable_vision" to true                 // 覆寫能力
    )
)
```
...
BreezeApp Engine 實作了一個完整的、生產就緒的 AI 推論系統，具備以下關鍵特性：

### ✅ **架構優勢**

- **Clean Architecture** 確保長期維護性
- **模組化設計** 支援靈活擴充
- **硬體抽象** 適應不同裝置能力
- **型別安全** 減少執行期錯誤
...
- **多 AI 能力** LLM、ASR、TTS、VLM 統一支援
- **串流處理** 即時響應，優質使用者體驗
- **安全機制** Guardian 內容過濾保障
- **智慧管理** 自動模型下載與記憶體最佳化

### ✅ **開發友善**

- **自描述參數** 零 UI 開發，自動生成設定介面  
- **豐富測試** 完整的單元測試與整合測試
- **詳細文件** 完整的開發指南與 API 文件
- **擴充性** 簡單的 Runner 擴充機制
...
這個架構為 Android 平台的 AI 應用提供了強大、靈活、可靠的基礎設施，支援從原型開發到生產部署的完整生命週期。

---

## 📚 延伸閱讀

- **[🧩 Runner Development Guide](../../docs/RUNNER_DEVELOPMENT.md)**: 學習如何開發自訂 Runner
- **[🏗️ Architecture Overview](../../docs/ARCHITECTURE.md)**: 深入理解系統架構設計
