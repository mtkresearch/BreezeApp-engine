...
import com.mtkresearch.breezeapp.engine.helpers.BreezeResponse
...
class BreezeAPIContractTest {
    ...
    fun chatResponse_matchesExpectedSchema() {
        val request = ChatRequest(
            messages = listOf(ChatMessage(role = "user", content = "@ai translate: 你好")),
            model = "breeze-llm"
        )
        ...
        // Validate parseable by Signal app
        val breezeResponse = BreezeResponse.fromJSON(json)
        assertNotNull("Signal app should be able to parse this", breezeResponse)
    }
    ...
    fun draftResponse_matchesExpectedSchema() {
        val request = ChatRequest(
            ...
            // though for this test the prompt content drives the "Tenant #1" logic in MockLLMRunner.
        ...
        assertFalse("draft_message should not be empty",
            json.getString("draft_message").isEmpty())
    }
    ...
    fun allResponseTypes_areComplete() {
        ...
        testCases.forEach { (command, expectedType) ->
            ...
            val json = JSONObject(response.text)
            assertEquals("Response type should be $expectedType",
                expectedType,
                json.getString("type"))
        }
    }
    ...
    private fun sendRequestAndWaitForResponse(request: ChatRequest): AIResponse {
        ...
        return AIResponse(
            ...
            state = if (result.error != null) AIResponse.ResponseState.ERROR else AIResponse.ResponseState.COMPLETED,
